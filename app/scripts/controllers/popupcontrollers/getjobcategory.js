  ...
  .controller('getjobCategoryCtrl',['$scope', '$rootScope','toastr', 'jobCategory','$timeout', 'locationService','documentService', 'jobListService',
    function ($scope, $rootScope, toastr, jobCategory, $timeout, locationService, documentService, jobListService){
      ...
      $scope.createJob = function() {
        ...
        $scope.partValidation = undefined;
      ...
      function addJobInfo(){
        ...
        if($scope.partsImage.length){
          ...
          for (var i=0; i < partsImages.length; i++) {
            if (partsImages[i].part_number) {
              var number = partsImages[i].image_ids.map(function (item) {
                return item['id'];
              });
              partsImages[i].image_ids = number;
              $scope.job.parts.push(partsImages[i]);
            }
            else{
              $scope.jobLoader = false;
              $scope.partValidation = "Please fill the blank part number";
              return;
            }
          }
